...
\title{Edge-Guided Panoramic Video Stitching with Limited Overlap\\
...
%guide according to the edge we mesh these video into grids frame by frame.
...
%In order to get more point matches and more accurate image alignment, we rectify the input videos and project them into a spherical coordinate system.
...
%For the points on the edge of each grid in the left image, we look up the nearest neighbor on the boundary within the corresponding grid in the image on the right.
...
%The interface shows the results of APAP, AutoPano and our method simultaneously.
...
\begin{figure}[t]
\centering
\includegraphics[scale=0.36]{picture62.png}
...
\begin{figure}[t]
\centering
\includegraphics[scale=0.36]{picture61.png}
