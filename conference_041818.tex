...
warped frame and calculating homegraphy matrix in each grid. (3) updating seam and stitching the origin videos to produce panoramic videos. The proposed method has proven to be
...
matched correctly because of the complicated scenes we captured. Under this condition, we propose to detect its edge and then match points according to the edge.
...
\caption{The spherically warped frame will be detected by HED \cite{xie2015holistically} firstly, and the edge detected frame mesh into $m \times n$ grids, then the overlapped region calculated its homegraphy matrix in grid $i$.
...
but ghosts will be produced when there is an object moving through the overlapped region. To stitch the videos seamlessly,
we adopt the overlaying strategy as in \cite{he2016parallax}. Firstly, we search a seam in the overlapping region according to our homegraphy matrices which was calculated in \ref{ssec:edge-detection}.
...
In the user study, 50 users use interface to score the videos. The videos show on the interface according to the rules: (1) the video groups were random and (2) the video types were random.
